...
//
// android/key.properties is gitignored: it holds the keystore password. Without
// it the build still works and falls back to the debug key, so a fresh clone can
// run the app — but a build meant for someone else's phone must be signed with
// the real key, or future updates cannot install over it.
...
android {
    ...
    compileOptions {
        // flutter_local_notifications uses java.time on APIs older than 26 and
    ...
    defaultConfig {
        ...
        applicationId = "br.com.felipearaujo.diarias"
    ...
    buildTypes {
        release {
            // The real key when key.properties is present; the debug key
            // otherwise, so `flutter run --release` still works on a clone that
            // has no keystore.
